...
with ventaMayor (vendedor, monto, ciudad) as
( select vendedorNombre, cantidad * precio, city
from tabla
group by vendedor, city)
select *
from ventaMayor
pivot(max(monto) FOR city in (Los angeles, Portland, Austin, Atlanta, Dallas))a
...
select id, display, location, rank()over(order by upvotes desc)as ranking, upvotes
from stack